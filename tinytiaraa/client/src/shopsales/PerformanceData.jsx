...
import { server } from '@/server'; // Ensure you have the correct server endpoint
import {
  Table,
  TableBody,
  TableCell,
...
const PerformanceDashboard = () => {
  ...
  if (!performanceData) {
    return  <div className="border border-gray-100 bg-white shadow-lg rounded-[10px]  space-y-4 p-4">
  ...
  const { metrics, data } = performanceData;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="text-center bg-white border border-gray-100 rounded-[4px] shadow-md p-4">
                ...
                <div className="relative group">
                    ...
                    <div className="w-[280px] absolute right-0 top-[90%] transform border border-gray-100 rounded-[4px] translate-y-[4px] hidden group-hover:block bg-white text-gray-800 text-[12px]  py-2 px-3 shadow-lg">
        ...
        <Table>
          ...
          <TableBody>
            {paginatedData.map((row, index) => (
              <TableRow key={index}>
                <TableCell><Link to={row.page} target='_blank'>{row.page}</Link></TableCell>
